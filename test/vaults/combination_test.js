...
describe('Сombination test (vaults)', () => {
  ...
  describe('Deposit test', () => {
    ...
    it('Deposit 0 ecr20', async() => {//@dev_vitalii поправить чтобы убрать возможность отравлять 0 + переделать тест
    ...
    it('Deposit 0 ecr20 if not approve', async() => {//@dev_vitalii поправить чтобы убрать возможность отравлять 0 + переделать тест
      let testdeposit = new bigNumber(0);
      let def_balance_sender = new bigNumber(await erc20.balanceOf(addr.sender));
      let def_yVault_balance = new bigNumber(await yVault.balance());
      let def_yVault_balance_erc =  new bigNumber(await erc20.balanceOf(yVault.address));
  ...
  describe('DepositAll test', () => {
    ...
    it('DepositAll work', async() => {//@dev_vitalii поправить чтобы убрать возможность отравлять 0 + переделать тест
      let def_balance_sender = new bigNumber(await erc20.balanceOf(addr.sender));
      await erc20.approve(yVault.address, (await erc20.balanceOf(addr.sender)).toString(),{ from: addr.sender});
      await yVault.depositAll( { from: addr.sender});
      expect((await erc20.balanceOf(addr.sender)).toString()).to.equal("0");
    ...
    it('DepositAll if 0 erc20', async() => {//@dev_vitalii поправить чтобы убрать возможность отравлять 0 + переделать тест
      let def_balance_sender = new bigNumber(await erc20.balanceOf(addr.arbitrary3));
      await erc20.approve(yVault.address, (await erc20.balanceOf(addr.arbitrary3)).toString(),{ from: addr.sender});
      await yVault.depositAll( { from: addr.arbitrary3});
  ...
  describe('setGovernance test', () => {
    ...
    it('Setter setGovernance', async() => {
      await yVault.setGovernance(addr.governance,{ from: addr.sender});
    ...
    it('setMin after setGovernance', async() => {
      ...
      yVault.setMin(min,{ from: addr.sender}),
      '!governance'
    );
    });
    it('setGovernance after setGovernance', async() => {
      ...
      expect((await yVault.governance()).toString()).to.equal(addr.governance.toString());
      await truffleAssert.reverts(
      yVault.setGovernance(addr.sender,{ from: addr.sender}),
      '!governance'
    );
    });
    it('setController after setGovernance', async() => {
      ...
      expect((await yVault.governance()).toString()).to.equal(addr.governance.toString());
      await truffleAssert.reverts(
      yVault.setController(controller.address,{ from: addr.sender}),
      '!governance'
    );
    });
